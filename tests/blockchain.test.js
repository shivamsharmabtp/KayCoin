...
describe("Blockchain class", function () {
  ...
  describe("wallet balance", function () {
    ...
    // stays the same.
    // Discussion: https://github.com/Savjee/KayCoin/pull/52
  ...
  describe("getAllTransactionsForWallet", function () {
    it("should get all Transactions for a Wallet", function () {
      ...
      assert.strict.equal(
        blockchain.getAllTransactionsForWallet("b2").length,
        2
      );
      assert.strict.equal(
        blockchain.getAllTransactionsForWallet(signingKey.getPublic("hex"))
          .length,
        5
      );
